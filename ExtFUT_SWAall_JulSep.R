...
print(dir.regions_1Input)
    ...
    calcSWA_JulSep <- function(RUN_DATA, name){
      #print("Pre d1")
      #print(Sys.time())
      # s=1
  ...
  getSWA_JS_FUT <- function(reg, name){
      #   sites <- list.files(dir.regions_3Runs[1])
      #   name=sites[1]
      #   reg=1
      ...
      dALL3 <- calcSWA_JulSep(RUN_DATA = runDataSC, name=name)
      ...
      for (cf in c(2:45)){
        ...
        dcf <- calcSWA_JulSep(RUN_DATA = runDataSC, name=name)
        dcf$scLEV <- flevs[cf]
        dcf$site <- name
...
#Try in parallel
 ...
 for (r in 1:5){
  ...
  annSWA_JulSep = foreach(s = sites, .combine = rbind) %dopar% {
    f <- list.files(file.path(dir.regions_3Runs[r], s) )

    if(length(f)==45){
      dsite <- getSWA_JS_FUT(reg = r, name=s)
      dsite[c(1:46),]
    }
  }
    stopCluster(cl)
  ...
  ifelse (r == 1, annualSWA_JulSepFUTURE <- annSWA_JulSep, annualSWA_JulSepFUTURE <- rbind(annualSWA_JulSepFUTURE, annSWA_JulSep))
}

names(annualSWA_JulSepFUTURE)[1:42] <- paste0("yr", c(1:42))
save(annualSWA_JulSepFUTURE, file=file.path(dir.jbHOME, "annualSWA_JulSepFUTURE"))
