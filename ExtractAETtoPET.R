...
#Function for calculating annual value
    ...
    getyearlyAETtoPET <- function(RUN_DATA, name){
      ...
      dAET <- as.data.frame(RUN_DATA@AET@Year)
      dPET <- as.data.frame(RUN_DATA@PET@Year)
      
      dAET[, 2] <- dAET[, 2]/dPET[, 2]
      
      
      names(dAET)[2] <- c(name)
      d3 <- as.data.frame(t(dAET))
...
for (r in 1:5){
  ...
  ifelse (r == 1, annualAETtoPET <- annAETtoPET, annualAETtoPET <- rbind(annualAETtoPET, annAETtoPET))
  names(annAETtoPET) <- paste(c(1915:2015))
}

names(annualAETtoPET) <- paste(c(1915:2015))
save(annualAETtoPET, file=file.path(dir.jbHOME, "annualAETtoPET19152015"))
