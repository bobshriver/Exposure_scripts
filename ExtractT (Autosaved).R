...
#Function for calculating annual value
    ...
    getyearlyTEMP <- function(RUN_DATA, name){
      d1 <- as.data.frame(RUN_DATA@TEMP@Year)
      #print(str(d1))
      d2 <- d1[,c(1, 4)]
      #print(str(d2))
      names(d2)[2] <- c(name)
...
for (r in 1:5){
  ...
  anntemp = foreach(s = sites, .combine = rbind) %dopar% {
    ...
    if(length(f)==1){
    load(file.path(dir.regions[r], s, "sw_output_sc1.RData"))
    d <- getyearlyTEMP(RUN_DATA = runDataSC, name=s)
    d[2,]
    }
  }
  ...
  ifelse (r == 1, annualtemp <- anntemp, annualtemp <- rbind(annualtemp, anntemp))
  names(anntemp) <- paste(c(1915:2015))
}

names(annualtemp) <- paste(c(1915:2015))
save(annualtemp, file=file.path(dir.jbHOME, "annualtemp19152015"))
