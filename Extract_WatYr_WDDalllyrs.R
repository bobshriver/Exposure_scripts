...
#Function for calculating WDD
    calcWatYrWDDall <- function(RUN_DATA, name){
      #print("Pre d1")
      #print(Sys.time())
      # s=8
      ...
      dVWC$Year[which(dVWC$DOY >273)] <- dVWC$Year[which(dVWC$DOY >273)] + 1
...
#Try in parallel
    ...
    for (r in 1:5){
      ...
      sites <- list.files(dir.regions_3Runs[r])
        ...
        ifelse (r == 1, WatYrwddALLlyrs <- WYwddALLlyrs, WatYrwddALLlyrs <- rbind(WatYrwddALLlyrs, WYwddALLlyrs))    
    }
...
names(WatYrwddALLlyrs) <- paste(c(1915:2015))
save(WatYrwddALLlyrs, file=file.path(dir.jbHOME, "WatYrwddALLlyrs19152015"))
