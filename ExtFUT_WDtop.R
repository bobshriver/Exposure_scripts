...
#Function for calculating annual value
  calcWETDAY_toplyrs <- function(RUN_DATA, name){
  ...
  getWDtopFUT <- function(reg, name){
      ...
      dALL3 <- calcWETDAY_toplyrs(RUN_DATA = runDataSC, name=name)
      ...
      for (cf in c(2:45)){
        ...
        dcf <- calcWETDAY_toplyrs(RUN_DATA = runDataSC, name=name)
        dcf$scLEV <- flevs[cf]
        dcf$site <- name
...
#Try in parallel
 ...
 for (r in 1:5){
  ...
  annWDtop = foreach(s = sites, .combine = rbind) %dopar% {
    f <- list.files(file.path(dir.regions_3Runs[r], s) )

    if(length(f)==45){
      dsite <- getWDtopFUT(reg = r, name=s)
      dsite[c(1:46),]
    }
  }
  ...
  ifelse (r == 1, annualWDtopFUTURE <- annWDtop, annualWDtopFUTURE <- rbind(annualWDtopFUTURE, annWDtop))
}

names(annualWDtopFUTURE)[1:42] <- paste0("yr", c(1:42))
save(annualWDtopFUTURE, file=file.path(dir.jbHOME, "annualWDtopFUTURE"))
