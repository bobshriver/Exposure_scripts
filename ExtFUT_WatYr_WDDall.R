...
#Function for calculating annual value
    calcWatYr_WDDall <- function(RUN_DATA, name){
      #print("Pre d1")
      #print(Sys.time())
      # s=8
  ...
  getWatYr_WDDallFUT <- function(reg, name){
      ...
      dALL3 <- calcWatYr_WDDall(RUN_DATA = runDataSC, name=name)
      ...
      for (cf in c(2:45)){
        ...
        dcf <- calcWatYr_WDDall(RUN_DATA = runDataSC, name=name)
        dcf$scLEV <- flevs[cf]
        dcf$site <- name
...
#Try in parallel
 ...
 for (r in 1:5){
  ...
  WatYrWDDall = foreach(s = sites, .combine = rbind) %dopar% {
    f <- list.files(file.path(dir.regions_3Runs[r], s) )

    if(length(f)==45){
      dsite <- getWatYr_WDDallFUT(reg = r, name=s)
      dsite[c(1:46),]
    }
  }
  ...
  ifelse (r == 1, WatYrWDDallFUTURE <- WatYrWDDall, WatYrWDDallFUTURE <- rbind(WatYrWDDallFUTURE, WatYrWDDall))
}

names(WatYrWDDallFUTURE)[1:42] <- paste0("yr", c(1:42))
save(WatYrWDDallFUTURE, file=file.path(dir.jbHOME, "WatYrWDDallFUTURE"))
