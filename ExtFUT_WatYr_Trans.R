...
#Function for calculating annual value
    ...
    getWatYrTRANSP <- function(reg, name){
      ...
      dAll1 <- as.data.frame(runDataSC@TRANSP@Month)
      ...
      dALL2 <- dALL2[,c("Year", "TotalLyrSum")]
      ...
      for (cf in c(2:45)){
        ...
        d1 <- as.data.frame(runDataSC@TRANSP@Month)
...
#Try in parallel
 ...
 for (r in 1:length(regions)){
  ...
  WatYrtransp = foreach(s = sites, .combine = rbind) %dopar% {
    ...
    if(length(f)==45){
      dsite <- getWatYrTRANSP(reg = r, name=s)
      dsite[c(1:46),]
    }
  }
  ...
  ifelse (r == 1, WatYrtranspFUTURE <- WatYrtransp, WatYrtranspFUTURE <- rbind(WatYrtranspFUTURE, WatYrtransp))
}

names(WatYrtranspFUTURE)[1:42] <- paste0("yr", c(1:42))
save(WatYrtranspFUTURE, file=file.path(dir.jbHOME, "WatYrtranspFUTURE"))
