...
print(dir.regions_1Input)
  ...
  getSWA_AJ_FUT <- function(reg, name){
      #   sites <- list.files(dir.regions_3Runs[1])
      #   name=sites[1]
      #   reg=1
      ...
      dALL3 <- calcSWA_AprJun(RUN_DATA = runDataSC, name=name)
      ...
      for (cf in c(2:45)){
        ...
        dcf <- calcSWA_AprJun(RUN_DATA = runDataSC, name=name)
        dcf$scLEV <- flevs[cf]
        dcf$site <- name
...
#Try in parallel
 ...
 for (r in 1:5){
  ...
  annSWA_AprJun = foreach(s = sites, .combine = rbind) %dopar% {
    f <- list.files(file.path(dir.regions_3Runs[r], s) )

    if(length(f)==45){
      dsite <- getSWA_AJ_FUT(reg = r, name=s)
      dsite[c(1:46),]
    }
  }
  ...
  ifelse (r == 1, annualSWA_AprJunFUTURE <- annSWA_AprJun, annualSWA_AprJunFUTURE <- rbind(annualSWA_AprJunFUTURE, annSWA_AprJun))
}

names(annualSWA_AprJunFUTURE)[1:42] <- paste0("yr", c(1:42))
save(annualSWA_AprJunFUTURE, file=file.path(dir.jbHOME, "annualSWA_AprJunFUTURE"))
