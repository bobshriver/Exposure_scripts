...
#  print(lp)
 ...
 #Function for calculating annual value
    getWatYrWETDAY_toplyrs <- function(RUN_DATA, name){
...
for (r in 1:5){
  ...
  WYwetday_top = foreach(s = sites, .combine = rbind) %dopar% {
    f <- list.files(file.path(dir.regions[r], s) )
    if(length(f)==1){
      load(file.path(dir.regions[r], s, "sw_output_sc1.RData"))
      d <- getWatYrWETDAY_toplyrs(RUN_DATA = runDataSC, name=s)
      d[2,]
    }
  }
  stopCluster(cl)
  ifelse (r == 1, WatYrwetday_top <- WYwetday_top, WatYrwetday_top <- rbind(WatYrwetday_top, WYwetday_top))    
...
names(WatYrwetday_top) <- paste(c(1915:2015))
save(WatYrwetday_top, file=file.path(dir.jbHOME, "WatYrwetday_top19152015"))
# 
