...
#Try in parallel
    ...
    for (r in 1:length(regions)){
      ...
      sites <- list.files(dir.regions_3Runs[r])
        ...
        ifelse (r == 1, annualHotDry_AprJun <- HotDry_AprJun, annualHotDry_AprJun <- rbind(annualHotDry_AprJun, HotDry_AprJun))    
    }
    

names(annualHotDry_AprJun) <- paste(c(1915:2015))
save(annualHotDry_AprJun, file=file.path(dir.jbHOME, "DDTemp_AprJun19152015"))
