...
print(dir.regions_1Input)
    ...
    calcHotDry_JanMar <- function(RUN_DATA, name){
      ...
      dVWC_JanMar <- dVWC[which(dVWC$Day %in% c(91:181)),]
      
      dVWC_JanMar$Temp <- dTemps[which(dTemps$Day %in% c(91:181)),5]
    ...
    for (r in 1:length(regions)){
      ...
      sites <- list.files(dir.regions_3Runs[r])
       ...
       registerDoParallel(cl)
        ...
        Below3DD_JanMar = foreach(s = sites, .combine = rbind,.packages=c('plyr','dplyr')) %dopar% {
          f <- list.files(file.path(dir.regions_3Runs[r], s) )
          if(length(f)==1){
            load(file.path(dir.regions_3Runs[r], s, "sw_output_sc1.RData"))
            print(s)
            d <- calcHotDry_JanMar(RUN_DATA = runDataSC, name=s)
            d
        ...
        ifelse (r == 1, annualBelow3DD_JanMar <- Below3DD_JanMar, annualBelow3DD_JanMar <- rbind(annualBelow3DD_JanMar, Below3DD_JanMar))    
    }
    
names(annualBelow3DD_JanMar) <- paste(c(1915:2015))
save(annualBelow3DD_JanMar, file=file.path(dir.jbHOME, "Below3DD_JanMar19152015.Rdata"))
