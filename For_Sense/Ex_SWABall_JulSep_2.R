      ...
      sites <- list.files(dir.regions_3Runs[r])
        ...
        SWA_JulSep = foreach(s = sites, .combine = rbind) %dopar% {
          ...
          if(length(f)==1){
            ...
            d <- calcSWA_JulSep(RUN_DATA = runDataSC, name=s)
            d[2,]
          }
        }
        stopCluster(cl)
        ...
        ifelse (r == 1, annualSWA_JulSep <- SWA_JulSep, annualSWA_JulSep <- rbind(annualSWA_JulSep, SWA_JulSep))    
    }
    

names(annualSWA_JulSep) <- paste(c(1915:2015))
save(annualSWA_JulSep, file=file.path(dir.jbHOME, "annualSWA_JulSep19152015_2"))
