...
print(dir.regions_1Input)
    ...
    calcHotDry_OctDec <- function(RUN_DATA, name){
      ...
      dVWC_OctDec <- dVWC[which(dVWC$Day %in% c(91:181)),]
      
      dVWC_OctDec$Temp <- dTemps[which(dTemps$Day %in% c(91:181)),5]
    ...
    for (r in 1:length(regions)){
      ...
      sites <- list.files(dir.regions_3Runs[r])
       ...
       registerDoParallel(cl)
        ...
        Below3DD_OctDec = foreach(s = sites, .combine = rbind,.packages=c('plyr','dplyr')) %dopar% {
          f <- list.files(file.path(dir.regions_3Runs[r], s) )
          if(length(f)==1){
            load(file.path(dir.regions_3Runs[r], s, "sw_output_sc1.RData"))
            print(s)
            d <- calcHotDry_OctDec(RUN_DATA = runDataSC, name=s)
            d
        ...
        ifelse (r == 1, annualBelow3DD_OctDec <- Below3DD_OctDec, annualBelow3DD_OctDec <- rbind(annualBelow3DD_OctDec, Below3DD_OctDec))    
    }
    
names(annualBelow3DD_OctDec) <- paste(c(1915:2015))
save(annualBelow3DD_OctDec, file=file.path(dir.jbHOME, "Below3DD_OctDec19152015.Rdata"))
